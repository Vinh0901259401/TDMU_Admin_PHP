...
try {
    ...
    // Tạo thông báo trước khi xóa câu hỏi
    ...
    $tieuDe = "Câu hỏi của bạn đã bị từ chối";
    $noiDung = "Câu hỏi '{$tieuDeCH}' của bạn không được phê duyệt và đã bị xóa khỏi hệ thống. Vui lòng kiểm tra lại nội dung câu hỏi và thử lại.";
    ...
    $stmtInsertNotification->execute([
        ...
        $tieuDe,
        $noiDung,
        $tk['ID'] // ID của người quản trị đang từ chối
    ]);
    ...
    // Xóa liên kết chủ đề câu hỏi
    $stmtDeleteLink = $conn->prepare("DELETE FROM cauhoi_chudecauhoi WHERE IDCauHoi = ?");
    $stmtDeleteLink->execute([$maCH]);
    
    // Xóa câu hỏi
    $stmtDeleteQuestion = $conn->prepare("DELETE FROM cauhoi WHERE ID = ?");
    $stmtDeleteQuestion->execute([$maCH]);
    ...
    echo json_encode([
        'code' => 200,
        'msg' => 'Câu hỏi đã bị từ chối và xóa thành công, đã gửi thông báo cho ' . $tenNguoiGui
    ]);
    ...
    error_log("Lỗi từ chối câu hỏi: " . $e->getMessage());
    
    echo json_encode([
        'code' => 500,
        'msg' => 'Đã xảy ra lỗi khi từ chối câu hỏi: ' . $e->getMessage()
    ]);
}
?>